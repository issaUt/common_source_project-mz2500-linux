...
既存のレジスタ表示コマンド `R` を、`Memory Bus(MZ2500)` を target にした場合に MZ-2500 の現在メモリ状態も表示するよう拡張しています。
...
既存のヘルプ表示コマンド `RH` を、`Memory Bus(MZ2500)` を target にした場合に統合デバッグ空間の説明を表示するよう拡張しています。
